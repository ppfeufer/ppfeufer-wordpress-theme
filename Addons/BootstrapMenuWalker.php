...
class BootstrapMenuWalker extends \Walker_Nav_Menu {
	...
	public function __construct() {
		...
		$this->eveApi = new Ppfeufer\Helper\EveApiHelper;
	}
	...
	public function start_el(&$output, $item, $depth = 0, $args = array(), $id = 0) {
		...
		if(\strcasecmp($item->attr_title, 'divider') == 0 && $depth === 1) {
			...
			$hasContent = true;
			if($item->post_parent !== 0 && Ppfeufer\Helper\PostHelper::getInstance()->hasContent($item->object_id) === false) {
				$hasContent = false;
				$class_names .= ' no-post-content';
			} // END if($item->post_parent !== 0 && Ppfeufer\Helper\PostHelper::getInstance()->hasContent($item->object_id) === false)
